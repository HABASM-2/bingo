...
def check_bingo_win(playboard, called_numbers):
    ...
    for r in range(5):
        if all(num in called for num in grid[r]):
            winning_cells.extend([(r, c) for c in range(5)])

    for c in range(5):
        if all(grid[r][c] in called for r in range(5)):
            winning_cells.extend([(r, c) for r in range(5)])

    if all(grid[i][i] in called for i in range(5)):
        winning_cells.extend([(i, i) for i in range(5)])

    if all(grid[i][4 - i] in called for i in range(5)):
        ...
        for number in remaining_numbers:
            ...
            for ws_id, user in game_state["users"].items():
                ...
                winning_cells = check_bingo_win(
                    user["playboard"], game_state["called_numbers"]
                )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({
            ...
            "playboard": user_state.get("playboard"),
        })
        ...
        while True:
            ...
            if data["type"] == "select_number":
                ...
                if num in user_state["playboard"] and num in game_state["called_numbers"]:
                    marked = user_state.get("marked_numbers", [])
                    if num not in marked:
                        marked.append(num)
                        user_state["marked_numbers"] = marked

                await websocket.send_json({
                    "type": "marked_numbers",
                    "marked_numbers": user_state.get("marked_numbers", []),
                })
        ...
        # AFTER GAME START → DISQUALIFY (NO REFUND)
            ...
            user_state["connected"] = False
            user_state["eligible"] = False   # 👈 CRITICAL LINE
